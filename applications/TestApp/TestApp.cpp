...
void TestAppAPP::Setup() {
  MLOGI("TestApp", "TestApp Started");
}

// Run in a loop after Setup()
void TestAppAPP::Loop() {
...
void TestAppAPP::KeyEventHandler(uint16_t keyID, KeyInfo* keyInfo) {
  ...
  {
    MLOGD("TestApp", "Key %d %d %d", xy.x, xy.y, keyInfo->state); // Print the key event to the debug log
...
void TestAppAPP::MidiEventHandler(MidiPacket midiPacket) {
...
void TestAppAPP::UIMenu() {
 ...
 // You can see the Note application for an example of how to do that. (Note Pad. Octave Shifter. Scales, ScaleVisualizer...)
  ...
  MLOGI("TestApp", "Exited UI Menu");
}

void TestAppAPP::End() {
  MLOGI("TestApp", "TestApp Exited");
}
